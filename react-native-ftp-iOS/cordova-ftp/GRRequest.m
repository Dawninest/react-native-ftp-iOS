...
- (instancetype)initWithDelegate:(id<GRRequestDelegate>)aDelegate datasource:(id<GRRequestDataSource>)aDatasource transferID:(NSString *)transferID requestDic:(NSDictionary*)requestDic
{
    self = [super init];
    if (self) {
		_passiveMode = YES;
        ...
        if (requestDic) {
            _requestDic = requestDic;//添加requestDic对象
        }
...
- (NSURL *)fullURL
{
    ...
    NSString *port = [self.dataSource portForRequest:self];
    ...
    NSString *fullURLString = [NSString stringWithFormat:@"%@%@:%@/%@", ftpPrefix, hostname, port,path];
